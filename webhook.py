...
# 1) Пытаемся построить RAG-индекс (не упадём, если rag.py/функции изменятся)
try:
    import rag  # ваш локальный модуль
...
# FAQ — гарантированно словарь {"topics": [...]}, даже если модель/парсинг не удался
faq_payload = {"topics": []}

# (Опционально) если задан OPENAI_API_KEY — попробуем получить FAQ от модели строго в JSON
...
# Пишем FAQ в любом случае корректным словарём
try:
    faq_path.write_text(json.dumps(faq_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"[INGEST] wrote: {faq_path} ({len(json.dumps(faq_payload, ensure_ascii=False))} bytes)")
except Exception as e:
